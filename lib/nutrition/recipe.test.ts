...
describe("自做饭菜 · 与内置库真实数据对得上", () => {
  it("一勺糖 = 份量表给的 8g × 白砂糖的定义性糖值 99.9% ≈ 8g 添加糖", () => {
    /*
     * ⚠️ 这条在 2026-09-22 之前钉的是**相反**的事实：那时内置库 250 多条一条都没补糖，
     * 于是按「没标 = 没加糖」的语义，一勺白砂糖贡献的糖是 0 —— 「做菜加的糖」这个入口是空的。
     *
     * 用户拍板后只给**纯糖类**补了糖值（白砂糖 99.9 / 蜂蜜 75.6），其余 248 条一条不动：
     *   ① 这两条的糖值是**定义性**的（蔗糖纯度、蜂蜜含糖量），不需要"查资料"，
     *      与「不影响现有体系」并不冲突 —— 它们本来就是糖；
     *   ② 不补的话，用户点了「＋ 一勺糖」看到的是「添加糖 0g」，那是个假数字。
     *
     * 仍然**不**做的事：不碰其余 248 条，也绝不用「碳水」推糖（纯糖能推，
     * 同一条判据套到米饭上就是假精确）。所以普通食物"没标糖"依旧等于"没加糖"。
     */
    const sugar = foodById("baitang");
    assert.ok(sugar);
    assert.equal(sugar.sugar, 99.9, "白砂糖是纯糖，这个值是定义性的");
    const r: MyRecipe = {
      id: "recipe-t3",
      name: "测试一勺糖",
      yieldG: 8,
    ...
    // 99.9 × 0.08 = 7.992 → 一位小数 8
    assert.equal(Math.round((values.sugar ?? 0) * 10) / 10, 8, "8g 白砂糖 ≈ 8g 添加糖");
