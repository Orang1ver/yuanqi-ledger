...
const LEAD_NOISE = [
  ...
  // 餐次那一组：既是时间词也是"整餐的说法"。剥的时候不许把整段剥空
  // （见 stripLeadNoise）——单独一句「午饭」要留给上层说「这是整餐的说法」。
  "早饭", "早餐", "午饭", "午餐", "晚饭", "晚餐", "宵夜", "早点",
  "今天", "昨天", "刚刚", "刚才", "然后", "后来",
 ...
 *
 * ⚠️ **餐次词剥空时不剥**。餐次词（早饭 / 午饭 / 晚饭 / 夜宵）既是噪音，也可能是用户
 * **真正想说的那件事**：「午饭吃了红烧肉」里的「午饭」该剥，而单独一句「午饭」
 * 剥空了就只剩"没说是吃什么"，上层那句更贴切的「这是整餐的说法」就永远说不出来了。
 *
 * 但这条**只对餐次词生效**：动词残渣（「我吃了」剥到只剩「吃了」）该剥空，
 * 剥空了上层会把这个片段丢掉 —— 它本来就不是一条记录。
 * 一刀切地"都不许剥空"会把「我吃了」变成一条记录（实测：三条变四条）。
 *
 * 补餐次词是因为实测漏了它们：「午饭吃了红烧肉」整段被当成食物名去模糊检索，
 * 再被 `matchFood` 那道"匹配要覆盖足够比例"的守卫拦下 ——
 * 结果是**本来认得的菜变成了认不得**。噪音表里本来就有「中午」「晚上」，
 * 缺的只是餐次那一组。
 */
const MEAL_ONLY_NOISE = new Set(["早饭", "早餐", "午饭", "午餐", "晚饭", "晚餐", "夜宵", "宵夜", "早点"]);
...
export function stripLeadNoise(fragment: string): string {
  ...
  for (let guard = 0; guard < 10; guard += 1) {
    ...
    if (!hit) break;
    const rest = out.slice(hit.length).trim();
    // 餐次词剥空就不剥：整句就是它，留给上层说「这是整餐的说法」
    if (!rest && MEAL_ONLY_NOISE.has(hit)) break;
    out = rest;
  }
  return out;
}
