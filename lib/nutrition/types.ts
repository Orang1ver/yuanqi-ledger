...
export type FoodItem = {
   ...
   * 添加糖 g/100g。
   *
   * ⚠️ **只由两个入口产生**：① 拍照识别（包装标签上单列的「糖」）
   * ② 做菜时加的糖（自做菜谱里那些 `baitang` 配料）。
   * **内置的二百多条一条都不补** —— 这个字段是给"新入口"用的，不是给全库回填的，
   * 所以它大面积为空是**预期状态**，不是"实现没做完"（界面措辞必须体现这一点）。
   *
...
export type DietEntry = {
  ...
  createdAt: number;
  /**
   * 这条记录是从**菜单库的哪道菜**记下来的（`TakeoutDish.id`）。
   *
   * ⚠️ **可选，而且永远可选**：1.2.0 之前的记录一条都没有这个字段，
   * 那是正常状态，不是数据坏了 —— 不要回填、不要因为缺它就跳过这条记录。
   *
   * 为什么加它：菜单库的一道菜在库里只是一行名字，**没有任何东西记得"我吃过它"**，
   * 于是「今天吃什么」只能凭空推，做不到"别又推昨天刚吃的"。
   * 加一个选项字段就能精确回答"最近吃过哪几道、各吃过几次"，
   * 而**不必另开一个 localStorage 键**（两个数据源必然漂移，而且新键还得登记进备份分组）。
   *
   * 一道「猜出来」的菜会展开成多条记录（按食材），这几条**带同一个 `dishId`** ——
   * 所以判断"吃过没有"要看的是这个字段，不是条数。
   */
  dishId?: string;
};
