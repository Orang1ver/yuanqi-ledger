...
> 状态：**已实现并上线（1.3.0 → 1.3.2），已真机端到端验证（两轮），回滚保险已三臂对照验证** —— 2026-09-19。
> 读法：**§1–§6 是最初的实施方案，保留原样**（它记录了当时为什么走 A 方案、以及预先定下的
> 判据）；**§7 是完成后的实测结论**，含两个只有真机才暴露的坑、回滚保险的对照实验、
> 以及一条**没复现**的观察（已如实标注，别当成结论）。
...
> ✅ **这条后来实测了**（见下「✅ 步骤 0 实测结果」）：SW **确实**仍控制着 OTA 页面、旧缓存也在，
> 但按 `sw.js` 的 fetch 分支逐条推演后危害很小（见 §2.5），所以**最终仍走 A**。
> 上面那句「从未验证过」是当时决策时的事实，保留不改。
...
{
  "version": "1.3.1",
  "apk": "apk/yuanqi-ledger-1.3.1.apk",
  "ota": {
    "base": "ota",
    "files": [
...
- 清单里只放 **真正给壳用的文件**：**排除** `version.json`、`sw.js`、点开头的文件
  （SW 见 §2.5，先排除、由 spike 结论再定）。
- `sha256` 由 deploy 时算（Node `crypto`），**不要手写**。
- 体积估算：86 个文件 ≈ 1.6MB。
- ⚠️ 老 APK 只读 `version` 与 `apk`，**多出字段是安全的** —— 但不要动既有字段。

#### ⚠️⚠️ 步骤 1 的原文有一处**根本性错误**，2026-09-19 真机端到端才发现

上面这段原来是「把 `out/` 里的文件登记进清单」—— **错**。`out/` 是**站点**产物，
而站点部署在子路径下；壳的 WebView 从 `https://localhost` 起，站点根就是 `/`。

拿站点产物去喂壳会发生什么（真机实测）：

1. 下载与校验**全部成功** —— 86 个文件、逐字节 sha256 一个不差
2. 点「立即更新」后基址也切了过去，HTML **真的被加载了**
3. 但 HTML 里引用的是 `/yuanqi-ledger/_next/...`，壳里这个路径不存在 → **全部 404**
4. 用户看到的是**一张裸 HTML**：内容在、样式没了、JS 没跑

**为什么所有本地闸门都放行**：构建全绿；清单 sha256 逐字节核对 86/86 全对；
连后来补的 `verify-ota-live` 也只查"清单与线上字节是否一致"。**「清单对」不等于「产物能在壳里跑」。**

**修正**（提交 `6bc6d20`）：

- `deploy.mjs` **构建两份产物**：先 `BASE_PATH=""` 出一份**壳内产物**（暂存到系统临时目录），
  再 `BASE_PATH=/yuanqi-ledger` 出站点产物，然后把壳内那份拷到站点 `out/ota/` 下，
  清单里加 **`ota.base = "ota"`**，壳按 `SITE/base/path` 下载。
- 新增 `assertRootBuild()`：**逐个文本文件扫子路径前缀**，发现就中止发布。
  判据与 `scripts/android/build-apk.ps1` 里守装机包的那段**同一套** ——
  两边喂给 WebView 的东西必须同构，一侧漏了就是这个下场。
- `lib/ota.ts` 的清单结构加 `base`（缺省=站点根，写了就必须是安全的相对路径）。
- 代价：**一次发布跑两次 `next build`**。想省掉只能改部署结构（把站点挪到根域名），
  **不要**想着"下载时把产物里的前缀字符串改写掉"——产物里合法地含有
  `https://orang1ver.github.io/yuanqi-ledger`（`lib/update.ts` 的 `SITE`），
  一刀切改写会把查更新的地址也改坏。
...
2. ⚠️ **WebView 里的 DOM 不给 `uiautomator` 看**（dump 出来只有一个 `WebView` 节点、`text=""`），
   只能 `input tap x y` 坐标点击。`screencap` 出的图就是**设备分辨率**（1080×2376），
   按"你看到的图"等比换算即可 —— ⚠️ 比例要用**图片的真实像素**去算，别按想当然的高度（会偏）。
   **原生控件（系统安装框等）是能 dump 的**，优先拿它的 `bounds`。
...
5. ⚠️ **vivo 的安装拦截对话框不认注入的点击**：`com.android.packageinstaller/.PackageInterceptActivity`
   会要求点一行小字「本次安装」，而 `input tap`（试了多个坐标）对它无效 ——
   这是防诈设计。**只能请用户在手机上点**，别在那儿耗时间。

**另：第一次带 OTA 的 APK 必须重装一次** —— 老 APK 里没有 OTA 代码，指望不上自更新。

### ✅ 端到端已验证（2026-09-19，vivo V2520A）

真机上走通了完整链路，**一共两轮**：

| 轮次 | 手机上的壳 | 线上版本 | 结果 |
|---|---|---|---|
| 1 | 首个带 OTA 的包 | 1.3.0 | 下载成功、切过去**裸样式** → 暴露"产物基址"问题（见步骤 1 那节） |
| 2 | 含 copy 修复的包 | 1.3.2 | **下载、切换、冷启动全部正常** |

第 2 轮的实测结论（都截了图）：

- 设置里显示「当前运行：v1.3.2（**更新版** 1.3.2）」—— 确实跑在 OTA 目录上，不是安装包内资源
- 切过去的页面**样式与脚本正常**（正是第 1 轮坏掉的地方）
- **冷启动后仍是更新版** —— 基址持久化生效
- 用户数据全在：体重 59.0kg、运动累计 1 次·2km、菜单库 12 道、水目标 2050ml

⚠️ **两轮是必须的，不是"保险起见"**：

- 有的 bug **只在"设备上已经有旧版本"时才触发**（增量复制要先建父目录，就是这一类）
- 而**壳代码的 bug，壳自己修不了自己** —— 只能出新的安装包

所以"装上第一版、更新一次就宣布 OTA 可用"是不够的。至少要 **装机 → 更新到下一版 →
再从那一版更新到再下一版**。

### ✅ 回滚保险已验证（2026-09-19，三臂对照）

第 1 轮冷启动确实退回了旧版，但「保险起作用」与「基址本来就没持久化成功」在外部观察上
**完全一样** —— 所以那不算证据。这次用一个 **debuggable 的测试包** + **对照臂**把它坐实。

做法（脚本已入库，用法 `bash scripts/android/ota-rollback-exp.sh 0|A|B|C|R`）：
让「坏基址确实持久化了」成为**已知条件**，再只改一个变量 —— 标记文件。

| 臂 | 持久化的基址 | `.pending` | 实测结果 |
|---|---|---|---|
| **A 对照** | `files/ota/9.9.9`（真的被加载，页面显示注入的 BROKEN 标记） | 无 | **什么都不发生**：基址不变、坏目录不隔离、logcat 无 `YuanqiOta` 行 |
| **B 处理** | 同上 | `1.3.2` | 基址改成 `files/ota/1.3.2`、坏目录改名 `9.9.9.tmp`、标记被删，**应用正常起来** |
| **C 处理** | 同上 | `0.0.0`（不存在） | 基址置**空串** → 回到打包资源，**应用正常起来** |

三重证据互相独立，各自都只有一方能产生：`logcat` 的 `YuanqiOta` 行（只有 Java 侧打）、
`run-as` 读回的 `serverBasePath` 实际值、`files/ota` 目录的改名实况（改名只有 Java 侧做）。

**A 臂是关键** —— 它证明「光是基址坏掉**不会**触发回滚」。少了它，B 的结果什么都说明不了。

出一个可调试包的办法：在 `android/app/build.gradle` 的 `release` 里临时加
`debuggable true`（用仓库正式密钥签名，所以 `adb install -r` **不丢数据**），
**构建完立刻删掉、别提交**。只有可调试的包，`adb shell run-as` 才读得到应用私有目录。

#### 顺带查清的两个 Capacitor 行为（记在这里免得再查一遍）

1. **基址写空串是对的，不是 bug。** `Bridge.java:301` 的判据是
   `path != null && !path.isEmpty() && new File(path).exists()` —— 空串被明确当作
   「不设基址 = 用打包资源」；Capacitor **自己**在二进制版本变化时也往这个键写空串
   （`Bridge.java:456`）。基址为空时 `getServerBasePath()` 仍返回 `"public"`，
   所以设置面板能正确显示「打包资源」。（先前怀疑过「空串会把应用卡在坏状态」，是错的。）
2. **装新 APK 后第一次启动会丢掉持久化基址 —— 这是 Capacitor 的设计，不是 OTA 坏了。**
   `isNewBinary()`（`Bridge.java:433`）发现包的 versionCode/Name 变了，就记下新的版本号
   并**把 `serverBasePath` 清成空串**，本次启动跳过持久化基址。意图是「新二进制自带的资源优先」，
   对用户是好事；但排障时会看到基址"自己没了"。
   **造实验状态时必须把 `lastBinaryVersionName/Code` 一起写**，否则观察分不清是谁干的。

#### 一个没复现的观察（如实记下，不当结论）

造状态的过程中，**更新过 APK 之后的头两次冷启动**里，首页出现过「数据看起来是空的」
（菜单库空、无饮食记录、水目标回到 2000ml）。**随后同一基址复跑即恢复**，且：

- leveldb 里只有 `https://localhost` **一个**来源 —— OTA 目录与打包资源**共用同一份存储**，
  OTA **不会**丢数据（这一点由复跑直接证实，不是推断）
- WebView 无崩溃记录（`app_webview/last-exit-info` 正常）

所以它**不是**来源隔离、也不是数据被清。倾向于「更新后头几次启动时 localStorage 尚未就绪」
一类的一次性现象，但**没复现，就别写成已验证的结论**。真要立规矩，方向是：
应用侧读到「存储为空」时**不要**顺手写默认值 —— 本项目已有的「菜单库只播种一次」原则正是这个方向。
...
| **回滚保险的对照实验（改过 Java 侧就重跑一遍）** | `scripts/android/ota-rollback-exp.sh`（用法 `0|A|B|C|R`） |
