...
> 状态：**已实现并上线（1.3.0 → 1.3.2），已真机端到端验证（两轮）** —— 2026-09-19。
> 读法：**§1–§6 是最初的实施方案，保留原样**（它记录了当时为什么走 A 方案、以及预先定下的
> 判据）；**§7 是完成后的实测结论**，含两个只有真机才暴露的坑和一条仍未闭合的验证。
...
⚠️ **仍未验证的一项**：回滚保险（新版起不来时自动退回上一版）到底有没有真的执行。
第 1 轮冷启动确实退回了旧版，但「保险起作用」与「基址本来就没持久化成功」
在外部观察上完全一样 —— 要坐实，得用**可调试**的测试包去看应用私有目录里的
标记文件与基址配置。**别把它写成已验证。**
