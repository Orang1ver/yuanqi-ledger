...
### ⚠️ 同批发现，但属 P6.1（数据层，仍未做）

- **「15只饺子」→ 15 份 3000g**（真实 300g）。`只` 在 `PORTION_UNITS` 里，但
  `data/foodPortions.json` **没有 `只` + 饺子 的规则**，于是走「份」兜底 200g × 15。
  「3只鸡腿」「10只虾」同理 —— 补规则要按食物逐条来，属 P6.1 第二批。
...
| 0.5.x / 0.6.0 | P5.3 份量档位点选 | PATCH 或 MINOR |
...
npm test                  # 当前 157 条（含 mealPresets、AI 推荐、口语解析边界）
