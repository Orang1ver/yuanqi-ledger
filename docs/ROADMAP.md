...
⚠️ **下一批被原料卡住**：裹粉的炸鸡排 / 烤冷面 / 小笼包要用**小麦面粉**，
猪脚饭要**猪蹄**，烤鸭饭要**烤鸭**，炒河粉要**河粉** —— 这些库里都没有，
硬凑就是编数字。补它们要回官方平台取数，而**该站的页面编号路径变过**
（现在首页是「预包装食品营养标签数据库」，`/fq/foodInfo/{id}.html` 已 404），
得先重新摸清入口再往下做。
...
| 1.0.0 | 及格线：P6.1 第二批剩下的（需先补面粉 / 猪蹄 / 河粉等原料）；P8 关键数据人工校核（需用户参与） | — |
| 1.0.0 | 及格线达成后发正式版 | — |
