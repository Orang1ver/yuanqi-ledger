...
> **2026-09-18 进展**：第 4 项（两位数）**已做**，并顺带修掉一个用户实测上报的切段 bug。
> 其余三项未做。

### ✅ 已完成（0.5.1）

- **「数字 + 量词」之间的空白**（用户实测上报）：在数字和量词之间打空格，
  「15 个饺子」会被切成 `["15", "个饺子"]`，界面上显示成**一条叫「5」的假食物 + 1 个饺子**（20g）。
  根因是 `glueMeasures` 的空白粘合只认**度量单位**（克 / g / ml）、不认**量词**（个 / 包 / 杯），
  于是那个空格被当成了「两样东西」的分隔。已补规则，并加了回归测试盯着
  「一包薯片 一杯奶茶」仍然切成两段。
...
- 单测 144 → 149 全绿。**纯解析改动，单测就够**（不经过界面，冒烟抓不到）。

### ❌ 仍未做（按优先级）

1. **「和 / 跟 / 还有 / 加 / 以及」切分**：让「米饭和红烧肉」解析成两条（最高频、最痛）。
2. **约数连写**：「两三」「三四」现在**不认** —— 退回单字「两」=2，剩下的「三个鸡蛋」进了食物名。
   要做到「取中值或保守值 + 标估算」才算完成。含糊相加（「两三」→ 5）比不认更糟，所以先不认。
3. **「一打」**：`打` 不在 `PORTION_UNITS` 里，也不是数字，完全不认。
4. **数字后置**：「饺子15个」现在只解析成 1 份（200g），真实是 15 个。

### ⚠️ 同批发现，但属 P6.1（数据层，不在本项）
...
- **红线**：守 `AGENTS.md` 地雷 15/19/20——静默算错是重罪，估不准就标「估算」、给区间，**绝不假装精确**。
- **改动**：`lib/nutrition/parse.ts`（`splitFragments`/`glueMeasures`、`CN_NUM_WORD`、`PORTION_UNITS`）+ 对应单测 + 探针验证。
- **版本**：PATCH。
...
| **0.5.1** ✅ | P5.4 部分：数字与量词间的空白、「十五」两位数（**已完成**，用户实测上报） | PATCH |
| 0.5.x / 0.6.0 | P5.3 份量档位 + P5.4 剩余（和/跟切分、约数、一打、数字后置） | PATCH |
